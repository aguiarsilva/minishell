...
static int check_identifier(const char *str)
{
    ...
    if (!str || !*str)
        return (0);
    if (!ft_isalpha(str[0]) && str[0] != '_') // First character must be alphabet or underscore
        return (0);
    i = 1; // Rest can be alphanumeric or underscore
    while (str[i])
    {
...
int unset_builtin(char **args, t_env **env_lst)
{
    ...
    exit_status = 0;
    if (!args || !args[0]) // Check if any arguments are provided
        return (0);  // No arguments is not an error
    i = 0;
    while (args[i])
    {
